...
public class ProgramatorDB implements ProgramatorRepository {
    ...
    @Override
    public Programator getAccount(String username, String parola, String functie) {
        logger.traceEntry("finding programator with {}", username);
        Connection con = jdbcUtils.getConnection();
        try(PreparedStatement preStmt = con.prepareStatement("select * from programator where username= ? and parola = ? and functie = ?")) {
            ...
            try(ResultSet result = preStmt.executeQuery()) {
                if(result.next()) {
                    ...
                    Programator programator = new Programator(username, parola, functie);
