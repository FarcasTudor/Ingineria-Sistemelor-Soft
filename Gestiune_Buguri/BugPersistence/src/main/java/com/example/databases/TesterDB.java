...
public class TesterDB implements TesterRepository {
    ...
    @Override
    public Tester getAccount(String username, String parola, String functie) {
        logger.traceEntry("finding tester with {}", username);
        Connection con = jdbcUtils.getConnection();
        try(PreparedStatement preStmt = con.prepareStatement("select * from tester where username= ? and parola = ? and functie = ?")) {
            ...
            try(ResultSet result = preStmt.executeQuery()) {
                if(result.next()) {
                    ...
                    Tester tester = new Tester(username, parola, functie);
                    tester.setId(id);
